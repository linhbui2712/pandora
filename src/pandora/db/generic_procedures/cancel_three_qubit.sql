create or replace procedure cancel_three_qubit(type_1 int, type_2 int, param_1 float, param_2 float, pass_count int, timeout int)
    language plpgsql
as
$$
declare
	 ...
	 while pass_count > 0 loop
        for gate in
            select * from linked_circuit
                     ...
                     and get_id_from_link(next_q2) = get_id_from_link(next_q3)
        ...
        loop
            ...
            first_id_plus_two := create_link(first.id, 2, first.type);
            ...
            if second.prev_q1 != first_id_plus_zero
                ...
                or second.prev_q3 != first_id_plus_two
            then
                commit;
                continue;
            end if;
            ...
            select * into e from linked_circuit where id = second_next_q3_id for update skip locked;
            select * into f from linked_circuit where id = first_prev_q3_id for update skip locked;

            -- Lock the 6 neighbours
            if a.id is null
                or b.id is null
                or c.id is null
                or d.id is null
            ...
            perform update_next_link(first_prev_q1_id, first.prev_q1, second.next_q1);
            perform update_next_link(first_prev_q2_id, first.prev_q2, second.next_q2);
            perform update_next_link(first_prev_q3_id, first.prev_q3, second.next_q3);
            perform update_prev_link(second_next_q1_id, second.next_q1, first.prev_q1);
            perform update_prev_link(second_next_q2_id, second.next_q2, first.prev_q2);
            perform update_prev_link(second_next_q3_id, second.next_q3, first.prev_q3);
